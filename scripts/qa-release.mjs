...
async function main() {
  ...
  const checks = [
    ...
    check("First-run QA script", packageJson.scripts?.["qa:first-run"] === "node scripts/qa-first-run.mjs", packageJson.scripts?.["qa:first-run"] || "missing"),
    ...
    check("Known limitations", /LoRA\/QLoRA/i.test(limitations) && /not a foundation\s+model/i.test(limitations) && /tiny from-scratch/i.test(limitations) && /Dry-run adapter receipts/i.test(limitations) && /Adapter Training Readiness/i.test(limitations) && /Adapter Training Operations Console/i.test(limitations) && /Trainer Preflight/i.test(limitations) && /Assisted Trainer Fix Loop/i.test(limitations) && /First Real Run Gate/i.test(limitations), "docs/KNOWN_LIMITATIONS.md"),
    ...
    check("README adapter readiness", /Adapter Training Readiness/i.test(readme) && /D-drive cache/i.test(readme) && /Transformers base model/i.test(readme), "README.md"),
    check("README adapter operations", /Adapter Training Operations Console/i.test(readme) && /Warm cache/i.test(readme) && /Retry op/i.test(readme), "README.md"),
    check("README trainer preflight", /Trainer Preflight/i.test(readme) && /safe(?:ly)? fall back to dry-run/i.test(readme) && /blocking check/i.test(readme), "README.md"),
    check("README assisted trainer fix loop", /Assisted Trainer Fix Loop/i.test(readme) && /Fix Trainer/i.test(readme) && /D-drive caches/i.test(readme) && /real tiny LoRA\/QLoRA/i.test(readme), "README.md"),
    check("README first real run gate", /First Real Run Gate/i.test(readme) && /First Real Run/i.test(readme) && /adapter-vs-base eval\s+receipt/i.test(readme), "README.md"),
    check("README adapter runner", /Adapter Training Run/i.test(readme) && /Run Trainer/i.test(readme) && /Promote AI/i.test(readme), "README.md")
  ];
